...
- **Categories** (fixed vocabulary, see `CONTEXT.md`): Command Echo, Tool Change Request, Swap (Load Sequence / Unload Sequence), Gate/TTG Map Update, Job State Change, MMU Statistics Report, Gate Statistics, Wear Counter, Warning (including Encoder Slippage), Error/Pause, EndlessSpool/Gate Remap, Spoolman, Uncategorized. Sync/Servo/Cutter/LED/eSpooler events are parsed and categorized (feeding relevant charts) but do not get dedicated Predefined Search buttons in v1.
...
- **Session detection**: Sessions are bounded by Job State transitions (`INITIALIZED`/`STARTED` through a terminal state like `READY`/`CANCELLED`). A session picker scopes every chart/diagram/raw-highlight to one Session or "Whole file".
